import {channelEntry,configureChannel} from './channel-state.mjs';
...
import {supportedGroups,groupCaveats,events,topics,prep,plan,ethicsQuestions,disclosure,caveats,deadlines} from './course.mjs';
...
const upcoming=()=>events(state.overrides,state.group||'C').filter(e=>Date.parse(e.start)>Date.now());
...
async function tickOne(){
 ...
 for(const item of dueReminders(events(state.overrides,state.group||'C'),state.receipts)){
  ...
  if(item.hours===24&&researchTopic){const key=`reading:${researchTopic}:${item.event.date}${(state.group||'C')==='C'?'':':'+state.group}`;await deliver(key,(item.event.topic==='UNKNOWN'?`Class topic unconfirmed. Optional reading for your next Group ${state.group||'C'} lab instead.\n`:'')+await research(researchTopic));}
 }
 for(const item of dueReminders(deadlines(state.group||'C'),state.receipts)){await deliver(item.key,`${item.hours}h reminder: ${item.event.topic} ${item.event.type} due ${item.event.date} at 16:00 America/Toronto. Derived from outline p. 3 and Group ${state.group||'C'} lab dates; check D2L amendments. Submit your own work to D2L using the required filename. Questions should be raised earlier than the final 36 hours.`);}
}
...
const groupOption={type:3,name:'group',description:'Lab group for this reminder channel',choices:supportedGroups.map(value=>({name:'Group '+value,value}))};
const command={name:'eche',description:'ECHE 3438 lab-group learning and reminders',options:[
 sub('setup','Choose this server’s course channel',[{type:7,name:'channel',description:'Study-group text channel',required:true,channel_types:[ChannelType.GuildText]},groupOption]),
 sub('group','Select the lab group for this channel (Manage Server)',[{...groupOption,required:true}]),
 sub('learn','Explore a manual chapter one step at a time',[{type:3,name:'topic',description:'Optional: defaults to your next lab experiment',required:false,choices:Object.entries(studies).map(([value,t])=>({name:t.title,value}))}]),sub('guide','Download summaries and readings for all six manual chapters'),sub('help','Show commands and how this planner works'),sub('plan','Download the complete course preparation plan'),sub('next','See the next class and selected group lab'),sub('questions','Questions and honest AI-use disclosure'),sub('prompt','Download the reusable Astra planning prompt'),sub('read','Find optional real-world reading'),sub('status','Check reminders and delivery health'),sub('pause','Pause reminders'),sub('resume','Resume reminders'),
...
async function connect(){
 ...
 client.on('interactionCreate',async i=>{
  if(i.isButton()&&i.customId.startsWith('study:')&&i.guildId&&channelEntry(persisted,i.guildId,i.channelId)){const [,code,n]=i.customId.split(':');const step=Number(n);if(studies[code]&&Number.isInteger(step)&&step>=0&&step<=5)await i.update(wizard(code,step)).catch(()=>{});return;}
  if(!i.isChatInputCommand()||i.commandName!=='eche'||!i.guildId)return;
  try{
   await i.deferReply({flags:MessageFlags.Ephemeral});
   const s=i.options.getSubcommand();
   if(['setup','group','pause','resume','topic','resolve'].includes(s)&&!i.memberPermissions?.has(P.ManageGuild)){await answer(i,'Manage Server permission is required to change reminders.');return;}
   if(s==='setup'){
    await serial(async()=>{
     ...
     const [,configured]=configureChannel(persisted,i.guildId,c.id,i.options.getString('group'));save();
     if(!cfg.guildId){cfg.guildId=i.guildId;cfg.channelId=c.id;fs.writeFileSync('config.json',JSON.stringify(cfg,null,2));}
     await answer(i,`Group ${configured.group} reminders configured in <#${c.id}> (${configured.paused?'paused':'active'}). Use /eche learn, /eche plan or /eche next there. Members with access to that channel can read reminders.`);
    });return;
   }
   const entry=channelEntry(persisted,i.guildId,i.channelId);const [key,g]=entry||[];
   ...
   await serial(()=>context.run({id:key,g},async()=>{
    if(s==='group'){state.group=i.options.getString('group',true);save();const lab=nextLab();await answer(i,`This channel now follows Group ${state.group}. 24h / 12h reminders, lab readings and derived deadlines use this group. Previous delivery history is preserved.\n${lab?prep(lab):'No remaining confirmed labs.'}`);return;}
    if(s==='learn'){const code=i.options.getString('topic')||nextLab()?.topic;if(studies[code])await i.editReply(wizard(code));else await answer(i,'Choose a topic with /eche learn topic:...; no upcoming lab chapter is configured.');}
    ...
    if(s==='help')await answer(i,'/eche group group:A|B|C|D - select lab group for this channel (Manage Server)\n/eche setup — choose this server’s course channel (Manage Server)\n/eche learn — guided chapter exploration\n/eche guide — all six chapter summaries and readings\n/eche next — next class and lab\n/eche plan — complete plan\n/eche prompt — reusable Astra prompt\n/eche read — optional article search\n/eche questions — technician questions\n/eche topic date:YYYY-MM-DD code:MF|LL|HU|AE — confirm a class topic\n/eche status, pause, resume\n\n24h and 12h reminders use America/Toronto. This is a source-based planner, not a live AI chat service. No model API cost.');
    if(s==='plan'||s==='prompt')await i.editReply({content:s==='plan'?'Complete plan and source limitations.':'Copy this prompt into Astra with your course documents.',files:[{attachment:Buffer.from(s==='plan'?plan(state.overrides,state.group||'C'):prompt(state.overrides,state.group||'C')),name:s==='plan'?'ECHE3438-plan.txt':'ASTRA-PROMPT.txt'}]});
...
function calendar(){const esc=s=>s.replace(/\\/g,'\\\\').replace(/\n/g,'\\n').replace(/,/g,'\\,').replace(/;/g,'\\;');const dt=s=>new Date(s).toISOString().replace(/[-:]/g,'').replace(/\.\d{3}/,'');return ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//ECHE3438//Lab planner//EN',...events(state.overrides,state.group||'C').flatMap(e=>['BEGIN:VEVENT',`UID:${e.id}@eche3438.local`,`DTSTAMP:${dt(Date.now())}`,`DTSTART:${dt(e.start)}`,`DTEND:${dt(Date.parse(e.start)+e.minutes*60000)}`,`SUMMARY:${esc(e.kind+': '+topics[e.topic].name)}`,`LOCATION:${e.room||topics[e.topic].room}`,`DESCRIPTION:${esc(prep(e))}`,...[24,12].flatMap(h=>['BEGIN:VALARM',`TRIGGER:-PT${h}H`,'ACTION:DISPLAY','DESCRIPTION:ECHE 3438 preparation','END:VALARM']),'END:VEVENT']),'END:VCALENDAR',''].join('\r\n');}
const server=http.createServer((req,res)=>{
 const pathname=new URL(req.url,'http://localhost').pathname;
 res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Cache-Control','no-store');
 if(pathname==='/api/status'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({status,lastError,group:state.group||'C',paused:state.paused,channel:cfg.channelId?`https://discord.com/channels/${cfg.guildId}/${cfg.channelId}`:null,events:upcoming(),deadlines:deadlines(state.group||'C'),topics,caveats:groupCaveats(state.group||'C'),receipts:state.receipts,configuredServers:new Set(Object.keys(persisted.guilds).map(k=>k.split(':')[0])).size,configuredChannels:Object.keys(persisted.guilds).length}));return;}
 if(['/plan.txt','/prompt.txt','/calendar.ics'].includes(pathname)){res.setHeader('Content-Type',pathname.endsWith('.ics')?'text/calendar':'text/plain; charset=utf-8');res.end(pathname==='/plan.txt'?plan(state.overrides,state.group||'C'):pathname==='/prompt.txt'?prompt(state.overrides,state.group||'C'):calendar());return;}
